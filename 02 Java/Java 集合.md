...
通过 HashMap 实现，HashMap 的 Key 即 HashSet 存储的元素，Value系统自定义一个名为PRESENT 的 Object 类型常量。
判断元素是否相同时，先比较hashCode，相同后再利用equals比较，查询O(1)。
...
使用动态数组实现，是容量可变的非线程安全列表，随机访问快，集合扩容时会创建更大的数组，把原有数组复制到新数组。
...
![jdk1.8 之前的内部结构-HashMap](https://javaguide.cn/assets/jdk1.7_hashmap.05c0a661.png)

相比于之前的版本， JDK1.8 之后在解决哈希冲突时有了较大的变化，当链表长度大于阈值（默认为 8）（将链表转换成红黑树前会判断，如果当前数组的长度小于 64，那么会选择先进行数组扩容，而不是转换为红黑树）时，将链表转化为红黑树，以减少搜索时间。
[#](https://javaguide.cn/java/collection/java-collection-questions-02.html#hashmap-的底层实现)
![jdk1.8之后的内部结构-HashMap](https://javaguide.cn/assets/jdk1.8_hashmap.ae1eae18.png)
...
![img](https://javaguide.cn/assets/treemap_hierarchy.d3d526de.png)
...
![Java 7 ConcurrentHashMap 存储结构](https://javaguide.cn/assets/java7_concurrenthashmap.49a48864.png)
